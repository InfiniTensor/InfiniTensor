...
template <class T, typename std::enable_if<std::is_base_of<GatherElementsObj, T>{},
                                           int>::type = 0>
void testGatherElementsCpu(
...
TEST(GatherElements, Cpu) {
    testGatherElementsCpu<GatherElementsObj>(IncrementalGenerator(), IncrementalGenerator(), 2, Shape{1, 2, 5, 5},
                               Shape{1, 2, 2, 5}, DataType::Float32);
    testGatherElementsCpu<GatherElementsObj>(IncrementalGenerator(), IncrementalGenerator(), 0, Shape{3, 5},
                               Shape{2, 5}, DataType::Float32);
    testGatherElementsCpu<GatherElementsObj>(IncrementalGenerator(), IncrementalGenerator(), 1, Shape{2, 2, 5, 5},
                               Shape{2, 1, 5, 5}, DataType::Float16);
}

#ifdef USE_CUDA
TEST(GatherElements, Cuda) {
    testGatherElementsCuda<GatherElementsObj>(IncrementalGenerator(), IncrementalGenerator(), 2, Shape{1, 2, 5, 5},
                               Shape{1, 2, 2, 5}, DataType::Float32);
    testGatherElementsCuda<GatherElementsObj>(IncrementalGenerator(), IncrementalGenerator(), 1, Shape{2, 2, 5, 5},
                               Shape{2, 1, 5, 5}, DataType::Float16);
}
#endif
