...
=======
                    ...
                    if p[0] != p[3] or p[1] != p[4] or p[2] != p[5]:
                        ...
                        adapt = node.input[0]
>>>>>>> origin/support_ascend_fp16_zyz
...
>>>>>>> origin/support_ascend_fp16_zyz
    ...
    def process_input(self, input_data):
        if self.model_name == "default_model_name":
            next(iter(self.inputs.values())).copyin_numpy(input_data)
        else:
    ...
    def optimize(self, model_name, input_shape, input_node) -> None:
        ...
        command = [
            ...
            '--input_shape', input_node + ':' + input_shape_str,
            '--data_dir', './data/' + model_name + '_data',
            '--data_types', 'float32',
            '--batch_num', '16'
        ]
        ...
        with open(log_file, 'w') as f:
            ...
            subprocess.run(atc_command, shell=False, stdout=f, stderr=f, check=True)
    ...
    def inference(self):
        ...
        if self.model_name == "default_model_name":
            self.handler.run()
            outputs = next(iter(self.outputs.values())).copyout_numpy()
        else:
            self.model_infer.inference()
            self.model_infer.get_result()
            outputs = self.model_infer.output_patch_list
    ...
    def release_resource(self):
        if self.model_name == "default_model_name":
            ...
            self.model_infer.release_resource()
