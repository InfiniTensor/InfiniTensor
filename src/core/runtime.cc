...
void collectShapeValueOps(
    ...
    if (!source) return ;
    IT_ASSERT(isShapeValueOp(source->getOpType()), "Runtime Rshape shape input unsupported");

    if (!requiredShapeOps.insert(source.get()).second) return ;

    if (source->getOpType() == OpType::Shape) return ;

    for (const Tensor &input : source->getInputs()) collectShapeValueOps(input, requiredShapeOps);
}

bool prepareRuntimeShapes(const Graph &graph, Device device, const RuntimeObj *runtime) {
    ...
    for (const Operator &op : graph->getOperators()) {
        ...
        if (!reshape->isRuntimeShape()) continue;

        hasDynamicReshape = true;
        collectShapeValueOps(reshape->getShapeTensor(), requiredShapeOps);
    }

    if (!hasDynamicReshape) return false;

    IT_ASSERT(device == Device::CPU, "Runtime Shape Tensor currently supports Native CPU only");
    ...
    for (const Operator &op : graph->getOperators()) {
        ...
        if (op->getOpType() != OpType::Reshape) continue ;
