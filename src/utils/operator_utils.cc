...
std::string execution_provider_to_str(ExecutionProvider provider) {
    switch (provider) {
    case ExecutionProvider::NativeCpu:
        return "NativeCpu";
    case ExecutionProvider::Infini:
        return "Infini";
    default:
        IT_TODO_HALT();
    }
...
std::string get_kernel_attrs_str(const KernelAttrs &kernelAttrs) {
    std::string deviceStr = execution_provider_to_str(std::get<0>(kernelAttrs));
