...
namespace infini {
    ...
    void saveTensorData(TensorObj* tensor, std::string file_path) {
        ...
        temp.mutable_shape()->CopyFrom({tensor->getDims().begin(), tensor->getDims().end()});
        temp.set_layout(data::LAYOUT_NHWC);
        temp.set_dtype(data::DTYPE_FLOAT);
        temp.mutable_data_float()->CopyFrom({tensor->getDataBlob()->getPtr<float*>(), tensor->getDataBlob()->getPtr<float*>() + tensor->size()});
        std::ofstream fileout(file_path, std::ios::out | std::ios::trunc | std::ios::binary);
