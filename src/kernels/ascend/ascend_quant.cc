...
class AscendQuantAclnn : public ASCENDKernelWithoutConfig {
    ...
    void compute(const Operator &_op,
        ...
        auto outputTensor =
            aclCreateTensor(outputDim.data(), outputDim.size(), ACL_INT8,
        ...
        auto ret = aclnnAscendQuantGetWorkspaceSize(
            inputTensor, scalesTensor, offsetTensor, op->getsqrtMode(),
            op->getroundMode().c_str(), 2, outputTensor, &workspaceSize,
            &executor);
        checkASCENDError(ret);
        ...
        ret = aclnnAscendQuant(workspaceAddr, workspaceSize, executor,
...
REGISTER_KERNEL(Device::ASCEND, OpType::AscendQuant, AscendQuantAclnn,
                "ascendquant_ASCEND_float");
} // namespace infini
