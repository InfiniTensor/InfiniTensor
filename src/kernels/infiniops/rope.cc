...
void buildRopeTables(size_t tableLen, size_t tableDim, size_t headDim,
    ...
    for (size_t pos = 0; pos < tableLen; ++pos) {
        for (size_t i = 0; i < tableDim; ++i) {
            const auto invFreq =
                std::pow(10000.0f, -static_cast<float>(2 * i) /
...
class RoPEInfiniOps : public infiniops::KernelWithoutConfig {
    void compute(const Operator &_op,
        ...
        ::infini::rt::TensorView::Strides ropeStrides{
            ...
            static_cast<::infini::rt::TensorView::Stride>(headDim), 1};
        ...
        auto config = infiniops::makeInfiniOpsConfig<
            ::infini::ops::RotaryEmbeddingInfinilm>(context);
        infiniops::dispatch::callRotaryEmbedding(handle, config, inputView,
                                                 posView, sinView, cosView,
                                                 false, outputView);
    }
};
...
REGISTER_KERNEL(ExecutionProvider::Infini, OpType::RoPE, RoPEInfiniOps,
                "RoPE_InfiniOps");
