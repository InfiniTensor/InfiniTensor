...
class GatherCpu final : public CpuKernelWithoutConfig {
    ...
    void doCompute(const Operator &_op) const {
        ...
        size_t inner = 1;
        for (size_t i = static_cast<size_t>(axis+1); i < dataShape.size(); ++i){
        ...
        for (size_t outerIndex = 0; outerIndex < outer; ++outerIndex){
            for (size_t indexOffset = 0; indexOffset < indices_count; ++indexOffset){
                int64_t index = static_cast<int64_t>(indexData[indexOffset]);

                IT_ASSERT(index >= -axisDim && index < axisDim, "Gather index is out of range");
                
                if (index < 0)
                    index += axisDim;
                
                const size_t inputElementOffset = (outerIndex * static_cast<size_t>(axisDim) + static_cast<size_t>(index)) * inner;
                const size_t outputElementOffset = (outerIndex * indices_count + indexOffset) * inner;
                std::memcpy(result + outputElementOffset * elementBytes, input + inputElementOffset * elementBytes, copyBytes);
            }
        }
    }
