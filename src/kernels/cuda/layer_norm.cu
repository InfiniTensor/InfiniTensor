...
void LaynormKernel(const float *input, const float *scale, const float eps,
    ...
    if (dimsize > 1024) {
        ...
        blockLaynormKernel<float, 1024>
            <<<num_block, BLOCK_DIM, 0, CUDAStream::stream>>>(
                input, scale, dimsize, stride, output, eps, scaleSize, bias,
                biasSize);
    } else if (dimsize > 31) {
        int BLOCK_DIM_x = 32;
        int BLOCK_DIM_y = 32;
        ...
        warpLaynormKernel<float, 32, 32>
            <<<grid_dim, block_dim, 0, CUDAStream::stream>>>(
                input, scale, dimsize, stride, output, eps, scaleSize,
                num_block, bias, biasSize);
    } else if (dimsize > 15) {
        int BLOCK_DIM_x = 16;
        int BLOCK_DIM_y = 64;
        int num_block_x = (num_block + BLOCK_DIM_y - 1) / BLOCK_DIM_y;
