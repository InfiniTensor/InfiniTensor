...
void LaynormKernel(const float *input, const float *scale, const float eps,
    ...
    if (dimsize > 1024) {
        ...
        blockLaynormKernel<float, 1024>
            <<<num_block, BLOCK_DIM, 0, CUDAStream::stream>>>(input, scale, dimsize, stride, output,
        ...
        warpLaynormKernel<float, 32, 32><<<grid_dim, block_dim, 0, CUDAStream::stream>>>(
        ...
        warpLaynormKernel<float, 16, 64><<<grid_dim, block_dim, 0, CUDAStream::stream>>>(
            input, scale, dimsize, stride, output, eps, scaleSize, num_block,
            bias, biasSize);
    } else if (dimsize > 7) {
        int BLOCK_DIM_x = 8;
        ...
        warpLaynormKernel<float, 8, 128><<<grid_dim, block_dim, 0, CUDAStream::stream>>>(
            input, scale, dimsize, stride, output, eps, scaleSize, num_block,
            bias, biasSize);
    } else {
        int BLOCK_DIM_x = 4;
        ...
        warpLaynormKernel<float, 4, 256><<<grid_dim, block_dim, 0, CUDAStream::stream>>>(
            input, scale, dimsize, stride, output, eps, scaleSize, num_block,
            bias, biasSize);
    }
}
...
void LaynormKernel(const float *input, const float *scale, const float eps,
    ...
    if (dimsize > 1024) {
        ...
        blockLaynormKernel<float, 1024><<<num_block, BLOCK_DIM, 0, CUDAStream::stream>>>(
...
void LaynormKernel(const half *input, const half *scale, const half eps,
    ...
    if (dimsize > 1024) {
        ...
        warpLaynormKernel<half, 32, 32><<<grid_dim, block_dim, 0, CUDAStream::stream>>>(
            input, scale, dimsize, stride, output, eps, scaleSize, num_block,
            bias, biasSize);
    } else if (dimsize > 15) {
        int BLOCK_DIM_x = 16;
        ...
        warpLaynormKernel<half, 16, 64><<<grid_dim, block_dim, 0, CUDAStream::stream>>>(
            input, scale, dimsize, stride, output, eps, scaleSize, num_block,
            bias, biasSize);
    } else if (dimsize > 7) {
        int BLOCK_DIM_x = 8;
        ...
        warpLaynormKernel<half, 8, 128><<<grid_dim, block_dim, 0, CUDAStream::stream>>>(
            input, scale, dimsize, stride, output, eps, scaleSize, num_block,
            bias, biasSize);
    } else {
        int BLOCK_DIM_x = 4;
        ...
        warpLaynormKernel<half, 4, 256><<<grid_dim, block_dim, 0, CUDAStream::stream>>>(
            input, scale, dimsize, stride, output, eps, scaleSize, num_block,
            bias, biasSize);
    }
}
...
void LaynormKernel(const half *input, const half *scale, const half eps,
    ...
    if (dimsize > 1024) {
        ...
        blockLaynormKernel<half, 1024><<<num_block, BLOCK_DIM, 0, CUDAStream::stream>>>(
            input, scale, dimsize, stride, output, eps, scaleSize);
    } else if (dimsize > 31) {
        int BLOCK_DIM_x = 32;
        int BLOCK_DIM_y = 32;
