...
//         j["type"] = 1;
//         j["data"] = std::make_tuple(algo, mode, fuseAct, time,
//         workspaceSize);
//     }
...
//             knDesc, cudnnDataType, CUDNN_TENSOR_NCHW, f, channelsPerGrp, r,
//             s));
//         // get bias
...
//             checkCudnnError(cudnnSetActivationDescriptor(
//                 actDesc, CUDNN_ACTIVATION_SIGMOID, CUDNN_NOT_PROPAGATE_NAN,
//                 0));
//             break;
//         case ActType::None:
//             checkCudnnError(
...
//     bool cuDNNUnfused(const Ref<ConvObj> &op, const ConvCuDnnPerfRecord
//     &record,
...
//         stat = cudnnConvolutionForward(context->cudnnHandle(), &alpha,
//         inDesc,
...
//     void compute(const Operator &op, const RuntimeObj *context) const
//     override {
//         auto record = make_ref<ConvCuDnnPerfRecordObj>(); // with paramters
//         in
//                                                           // default ctor
...
//                 record.algo = algo;
//                 cudnnStatus_t stat;
//                 const auto &[inData, knData, outData, inDesc, knDesc,
//                 biasDesc,
//                              convDesc, actDesc, outDesc] =
...
//                 record.time = timeit(
//                     [&]() {
//                         cudnnConvolutionForward(context->cudnnHandle(),
//                         &alpha,
//                                                 inDesc, inData, knDesc,
//                                                 knData, convDesc,
//                                                 ALGOS[record.algo], wsData,
//                                                 record.workspaceSize, &beta,
//                                                 outDesc, outData);
//                     },
//                     [&]() { context->sync(); });
...
//   public:
//     convCudnn() {
//         auto computePtr = [this](const Operator &op, const PerfRecord
//         &record,
...
namespace infini {
class ConvNineth : public CudaKernelWithoutConfig {
    void compute(const Operator &_op,
        ...
        launch_conv2d(CUDAStream::getCurrentStream(), input, weight, bias,
                      output, NineToothedTensor{}, NineToothedTensor{}, n, c, h,
                      w, f, r, s, sh, sw, ph, pw, dh, dw,
                      NineToothedDataType::NINETOOTHED_FLOAT32);
    }
};

REGISTER_KERNEL(Device::CUDA, OpType::Conv, ConvNineth, "Conv_Nineth_CUDA");
} // namespace infini
