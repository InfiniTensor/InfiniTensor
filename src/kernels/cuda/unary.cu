...
__global__ void _softmax_kernel1(float *input, float *sum_output, int n) {
    float sum = 0.0f;
...
__global__ void _softmax_kernel2(float *input, float sum, float *output, int n) {
...
void softmax_kernel(float *input, float *output, int num) {
    ...
    _softmax_kernel1<<<1, 1>>>(input, &sum, num);
    _softmax_kernel2<<<blocksize, gridsize>>>(input, sum, output, num);
}
