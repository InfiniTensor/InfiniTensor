...
template <class T>
__global__ void _sub_kernel(void *x, void *y, void *z, int a0, int a1, int a2,
                            int a3, int b0, int b1, int b2, int b3, int c0,
                            int c1, int c2, int c3) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
...
template <class T>
__global__ void _greater_kernel(void *x, void *y, void *z, int a0, int a1,
...
template <class T>
__global__ void _greater_or_equal_kernel(void *x, void *y, void *z, int a0,
...
template <class T> __device__ bool compare(T a, T b, T epsilon = 1e-9) {
...
__global__ void _equal_kernel(void *x, void *y, void *z, int a0, int a1, int a2,
                              int a3, int b0, int b1, int b2, int b3, int c0,
                              int c1, int c2, int c3, T epsilon = 1e-9) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
    int n = c0 * c1 * c2 * c3;
    ...
    for (int i = index; i < n; i += stride) {
        ...
        if constexpr (std::is_floating_point<T>::value) {
            result =
                compare(((T *)x)[a0_index * a1 * a2 * a3 + a1_index * a2 * a3 +
                                 a2_index * a3 + a3_index],
                        ((T *)y)[b0_index * b1 * b2 * b3 + b1_index * b2 * b3 +
                                 b2_index * b3 + b3_index],
                        epsilon);
        } else {
            result = ((T *)x)[a0_index * a1 * a2 * a3 + a1_index * a2 * a3 +
                              a2_index * a3 + a3_index] ==
                     ((T *)y)[b0_index * b1 * b2 * b3 + b1_index * b2 * b3 +
                              b2_index * b3 + b3_index];
        }
...
void sub_kernel(int dType, void *a, void *b, void *c, int a0, int a1, int a2,
                int a3, int b0, int b1, int b2, int b3, int c0, int c1, int c2,
                int c3) {
...
void greater_kernel(int dType, void *a, void *b, void *c, int a0, int a1,
                    int a2, int a3, int b0, int b1, int b2, int b3, int c0,
                    int c1, int c2, int c3) {
    int blocksize = block_work_size();
    int num = c0 * c1 * c2 * c3;
    int gridsize = (num + block_work_size() - 1) / block_work_size();
    SWITCH_DTYPE(greater, dType)
}

void greater_or_equal_kernel(int dType, void *a, void *b, void *c, int a0,
                             int a1, int a2, int a3, int b0, int b1, int b2,
                             int b3, int c0, int c1, int c2, int c3) {
    int blocksize = block_work_size();
    int num = c0 * c1 * c2 * c3;
    int gridsize = (num + block_work_size() - 1) / block_work_size();
    SWITCH_DTYPE(greater_or_equal, dType)
}
