...
template <class T> __inline__ __device__ T blockReduceSum(T val) {
...
template <class T>
__global__ void _rmsnorm_kernel(void *in, void *weight, void *out,
    ...
    for (int idx = threadIdx.x; idx < hidden_size; idx += blockDim.x) {
        ...
        ((T *)out)[blockIdx.x * hidden_size + idx] =
            ((T)(x * s_variance)) * ((T *)weight)[idx];
    }
}
