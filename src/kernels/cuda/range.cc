...
class RangeCuda : public CudaKernelWithoutConfig {
    void compute(const Operator &_op,
        ...
        int size =
            std::max(std::ceil((limitData - startData) / deltaData), 0.0f);

        range_kernel(startData, limitData, deltaData, (float *)outputData,
                     size);
    }
};

REGISTER_KERNEL(Device::CUDA, OpType::Range, RangeCuda, "Range_CUDA_Int32");
