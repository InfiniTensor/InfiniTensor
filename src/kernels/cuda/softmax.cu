...
void softmax_kernel(int num_blocks, Tdata *input, Tdata *output, int size,
    ...
    if (dimsize > 1024 * 128) {
        ...
        int BLOCK_DIM = 1024;
        _blockSoftmaxKernel<Tdata, 1024>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 1024 * 64) {

        int BLOCK_DIM = 1024;
        _blockSoftmaxKernel<Tdata, 1024, 128>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 1024 * 32) {

        int BLOCK_DIM = 1024;
        _blockSoftmaxKernel<Tdata, 1024, 64>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 1024 * 16) {

        int BLOCK_DIM = 1024;
        _blockSoftmaxKernel<Tdata, 1024, 32>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 1024 * 4) {

        int BLOCK_DIM = 1024;
        _blockSoftmaxKernel<Tdata, 1024, 16>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 1024) {

        int BLOCK_DIM = 1024;
        _blockSoftmaxKernel<Tdata, 1024, 4>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 512) {
        int BLOCK_DIM = 1024;
        _blockSoftmaxKernel<Tdata, 1024, 1>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 256) {
        int BLOCK_DIM = 512;
        _blockSoftmaxKernel<Tdata, 512, 1>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 128) {
        int BLOCK_DIM = 256;
        _blockSoftmaxKernel<Tdata, 256, 1>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 64) {
        int BLOCK_DIM = 128;
        _blockSoftmaxKernel<Tdata, 128, 1>
            <<<num_blocks, BLOCK_DIM, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 31) {
        int BLOCK_DIM_x = 32;
        ...
        _warpSoftmaxKernel<Tdata, 32, 32, 2>
            <<<grid_dim, block_dim, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 15) {
        int BLOCK_DIM_x = 16;
        ...
        _warpSoftmaxKernel<Tdata, 16, 64, 2>
            <<<grid_dim, block_dim, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else if (dimsize > 7) {
        int BLOCK_DIM_x = 8;
        ...
        _warpSoftmaxKernel<Tdata, 8, 128, 2>
            <<<grid_dim, block_dim, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    } else {
        int BLOCK_DIM_x = 4;
        ...
        _warpSoftmaxKernel<Tdata, 4, 256, 2>
            <<<grid_dim, block_dim, 0, CUDAStream::getCurrentStream()>>>(
                input, output, size, dimsize, stride);
    }
}

template void softmax_kernel<float>(int num_blocks, float *input, float *output,
                                    int size, int dimsize, int stride);
template void softmax_kernel<half>(int num_blocks, half *input, half *output,
                                   int size, int dimsize, int stride);
} // namespace infini
