...
class AbsXdnn : public XPUKernelWithoutConfig {
    void compute(const Operator &_op,
        ...
        auto ret = baidu::xpu::api::abs<float>(context->XPUHandle(), (float*)aData, (float*)cData, len);
        assert(ret == 0);
        return;
    }
};
...
class ATanXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op,
        ...
        auto ret = baidu::xpu::api::arctan<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
        assert(ret == 0);
        return;
    }
};
...
class LogXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op,
        ...
        auto ret = baidu::xpu::api::log<float>(context->XPUHandle(), (float *)aData, (float *)temp, len);
        // get ptr of divider
        XPUPtr dd = (float* )(context->getWorkspace((1 + len) * sizeof(float))) + len * sizeof(float);
        ...
        case LogObj::LogE:
            ...
            ret = baidu::xpu::api::copy<float>(context->XPUHandle(), (float *)temp, (float *)cData, len);       
            break;
        case LogObj::Log2:
            constant = std::log(2);
            context->copyBlobFromCPU(dd, &constant, sizeof(float));
            ret = baidu::xpu::api::broadcast_div<float>(context->XPUHandle(), (float *)temp, 
                            (float *)dd, (float *)cData, aDim, divDim);
            break;
        case LogObj::Log10:
            constant = std::log(10);
            context->copyBlobFromCPU(dd, &constant, sizeof(float));
...
class CosXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::cos<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class SinXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::sin<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class TanXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::tan<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class SinhXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::sinh<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class CoshXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::cosh<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class ErfXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::erf<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class ACosXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::arccos<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class ACoshXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::acosh<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class ASinXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::arcsin<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class ASinhXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::asinh<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
class ATanhXdnn : public XPUKernelWithoutConfig{
    void compute(const Operator &_op, const RuntimeObj *_context) const override {
        ...
        void *const cData = (op->getOutput()->getRawDataPtr<void *>());
        auto len = op->getInputs(0)->size();
        auto ret = baidu::xpu::api::atanh<float>(context->XPUHandle(), (float *)aData, (float *)cData, len);
...
REGISTER_KERNEL(Device::XPU, OpType::Log, DataType::Float32, LogXdnn,
                "Log_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Cos, DataType::Float32, CosXdnn, "Cos_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Sin, DataType::Float32, SinXdnn, "Sin_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Tan, DataType::Float32, TanXdnn, "Tan_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Sinh, DataType::Float32, SinhXdnn, "Sinh_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Cosh, DataType::Float32, CoshXdnn, "Cosh_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Erf, DataType::Float32, ErfXdnn, "Erf_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Acos, DataType::Float32, ACosXdnn, "ACos_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Acosh, DataType::Float32, ACoshXdnn, "ACosh_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Asin, DataType::Float32, ASinXdnn, "ASin_xdnn_Float32");
REGISTER_KERNEL(Device::XPU, OpType::Asinh, DataType::Float32, ASinhXdnn, "ASinh_xdnn_Float3 2");
REGISTER_KERNEL(Device::XPU, OpType::Atanh, DataType::Float32, ATanhXdnn, "ATanh_xdnn_Float32");
}; // namespace infini
