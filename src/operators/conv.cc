...
//     n = input->getDims()[0], c = input->getDims()[1], h =
//     input->getDims()[2], w = input->getDims()[3], f = weight->getDims()[0], r
//     = weight->getDims()[2], s = weight->getDims()[3]; if (mode ==
//     PaddingMode::Same) {
//         int oh = h / sh;
//         int ow = w / sw;
...
//                  ActType act)
//     : ConvBaseObj(OpType::Conv, {input, weight}, output, ph, pw, sh, sw, dh,
//     dw,
//                   input, weight, act) {
//     if (bias)
//         IT_TODO_HALT();
...
// ConvObj::ConvObj(GraphObj *graph, Tensor input, Tensor weight, Tensor output,
//                  PaddingMode mode, int sh, int sw, int dh, int dw, Tensor
//                  bias, ActType act)
//     : ConvBaseObj(OpType::Conv, {input, weight}, output, mode, sh, sw, dh,
//     dw,
//                   input, weight, act) {
//     if (bias)
//         IT_TODO_HALT();
...
//                      int sw, int dd, int dh, int dw, Tensor bias, ActType
//                      act)
//     : ConvBaseObj(OpType::Conv3d, {input, weight}, output, ph, pw, sh, sw,
//     dh,
//                   dw, input, weight, act),
//       pd(pd), sd(sd), dd(dd) {
//     if (bias)
...
//     : ConvBaseObj(OpType::Conv3d, {input, weight}, output, mode, sh, sw, dh,
//     dw,
//                   input, weight, act),
//       sd(sd), dd(dd) {
//     if (bias)
...
//                                          Tensor weight, Tensor output, int
//                                          ph, int pw, int sh, int sw, int dh,
...
//     : ConvBaseObj(OpType::ConvTranspose, {input, weight}, output, mode, sh,
//     sw,
...
//     const Tensor &input = inputs[0];
//     const Tensor &weight = inputs[1];
//     n = input->getDims()[0], f = input->getDims()[1], h =
//     input->getDims()[2], w = input->getDims()[3], c = weight->getDims()[1], r
//     = weight->getDims()[2], s = weight->getDims()[3]; if (mode ==
//     PaddingMode::Same) {
//         int oh = h / sh;
//         int ow = w / sw;
...
//     h = inputX->getDims()[2], w = inputX->getDims()[3], f =
//     diffY->getDims()[0], r = diffY->getDims()[2], s = diffY->getDims()[3]; if
...
//                                              Tensor diffY, Tensor diffW, int
//                                              ph, int pw, int sh, int sw, int
//                                              dh, int dw, Tensor bias, ActType
//                                              act)
//     : ConvBaseObj(OpType::Conv, {inputX, diffY}, diffW, ph, pw, sh, sw, dh,
//     dw,
//                   inputX, diffY),
//       act(act) {
//     if (bias)
...
//                                              PaddingMode mode, int sh, int
//                                              sw, int dh, int dw, Tensor bias,
//                                              ActType act)
...
// ConvTransposed2dNHWCObj::ConvTransposed2dNHWCObj(GraphObj *graph, Tensor
// input,
//                                                  Tensor weight, Tensor
//                                                  output, int ph, int pw, int
//                                                  sh, int sw, int dh, int dw,
...
// input,
//                                                  Tensor weight, Tensor
//                                                  output, PaddingMode mode,
//                                                  int sh, int sw, int dh, int
//                                                  dw, int oph, int opw, int
//                                                  group, Tensor bias, ActType
//                                                  act)
//     : ConvBaseObj(OpType::ConvTranspose, {input, weight}, output, mode, sh,
//     sw,
//                   dh, dw, output, weight, act),
...
//     const Tensor &input = inputs[0];
//     const Tensor &weight = inputs[1];
//     n = input->getDims()[0], f = input->getDims()[3], h =
//     input->getDims()[1], w = input->getDims()[2], c = weight->getDims()[3], r
//     = weight->getDims()[1], s = weight->getDims()[2]; if (mode ==
//     PaddingMode::Same) {
//         int oh = h / sh;
//         int ow = w / sw;
//         ph = (h - oh * sh + (r - sh) * dh) / 2;
...
namespace infini {
ConvObj::ConvObj(GraphObj *graph, Tensor input, Tensor weight, Tensor bias,
                 Tensor output, vector<int> pad_, vector<int> stride_,
