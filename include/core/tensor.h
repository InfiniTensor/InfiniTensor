...
class TensorObj : public TensorBaseObj {
  ...
  public:
    ...
    NineToothedTensor ToNineToothedTensor(Shape dim = {}) const {
        NineToothedTensor nt;

        int ndim = shape.size();

        // 1️⃣ 分配 shape 和 strides
        nt.shape = (uint64_t *)malloc(ndim * sizeof(uint64_t));
        nt.strides = (int64_t *)malloc(ndim * sizeof(int64_t));
        ...
        if (dim.size() != 0) {
            ...
            // 2️⃣ 计算 strides (行优先紧凑存储)
            if (ndim > 0) {
                nt.strides[ndim - 1] = 1;
                for (int i = ndim - 2; i >= 0; --i) {
                    nt.strides[i] = nt.strides[i + 1] * nt.shape[i + 1];
                }
            }
        }

        // 3️⃣ 数据指针直接指向 TensorObj 内部 Blob
        nt.data = getRawDataPtr<void *>();
