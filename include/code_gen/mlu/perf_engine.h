...
class PerfEngine {
  ...
  public:
    ...
    ~PerfEngine() {
        ...
        dumpPerfData();
        checkBangError(cnrtFree(inputPtr));
        checkBangError(cnrtFree(weightPtr));
        checkBangError(cnrtFree(outputPtr));
        checkBangError(cnrtFree(workspace));
    ...
    /////cublasHandle_t cublasHandle() const { return cublas; }
    double cublasHandle() const { return cublas; }

    double getConvAlgo(const ConvArgs &args) {
        return convPerf.at(args).algo;
    }
    double getConvTransAlgo(const ConvArgs &args) {
        return convTransPerf.at(args).algo;
    }
    double getMatmulAlgo(const MatmulArgs &args) {
        return matmulPerf.at(args).algo;
    }
