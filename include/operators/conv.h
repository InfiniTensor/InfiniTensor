...
//  * (height), and W (width) respectively; The weight tensor has four
//  dimensions,
//  * called F (number of filters), C (channel), R (height of weight), and S
//  (width
//  * of weight) respectively; The output tensor has four dimensions, called N,
//  F,
...
//  * means the output has the same shape as the input. Valid means padding size
//  is
...
//  * Stride is assigned by sh and sw, denoting the stride along height
//  dimension
...
//      */
//     ConvBaseObj(OpType opType, TensorVec inputs, Tensor &output, int ph, int
//     pw,
...
//   public:
//     ConvObj(GraphObj *graph, Tensor input, Tensor weight, Tensor output, int
//     ph,
...
//                           Tensor diffW, int ph, int pw, int sh = 1, int sw =
//                           1, int dh = 1, int dw = 1, Tensor bias = nullptr,
//                           ActType act = ActType::None);
//     // Constructors for setting padding mode
//     ConvBackwardFilterObj(GraphObj *graph, Tensor inputX, Tensor diffY,
//                           Tensor diffW, PaddingMode mode = PaddingMode::Same,
//                           int sh = 1, int sw = 1, int dh = 1, int dw = 1,
//                           Tensor bias = nullptr, ActType act =
//                           ActType::None);
...
//                         Tensor output, int ph, int pw, int sh = 1, int sw =
//                         1, int dh = 1, int dw = 1, int oph = 0, int opw = 0,
...
//                             int opw = 0, int group = 1, Tensor bias =
//                             nullptr, ActType act = ActType::None);
//     // Constructors for setting padding mode
//     ConvTransposed2dNHWCObj(GraphObj *graph, Tensor input, Tensor weight,
//                             Tensor output, PaddingMode mode =
//                             PaddingMode::Same, int sh = 1, int sw = 1, int dh
//                             = 1, int dw = 1, int oph = 0, int opw = 0, int
//                             group = 1, Tensor bias = nullptr, ActType act =
//                             ActType::None);
...
class ConvObj : public OperatorObj {
  ...
  public:
    ConvObj(GraphObj *graph, Tensor input, Tensor weight, Tensor bias,
